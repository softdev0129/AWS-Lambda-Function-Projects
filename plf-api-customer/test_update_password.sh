...
#  -d "{\"path\": \"/customer/auth/register\", \"body\": \"{\\\"email\\\": \\\"${EMAIL}\\\", \\\"password\\\": \\\"${PASSWORD}\\\"}\"}")
#
#REGISTER_BODY=$(echo "${REGISTER_RESPONSE}" | sed '$d')
#REGISTER_STATUS=$(echo "${REGISTER_RESPONSE}" | tail -n1)
#
#echo "Register Response Body: ${REGISTER_BODY}"
#echo "Register Response Status: ${REGISTER_STATUS}"
# Check if registration was successful
  ...
  LOGIN_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "http://localhost:$LOCAL_PORT/2015-03-31/functions/function/invocations" \
    -d "{\"path\": \"/customer/auth/login\", \"body\": \"{\\\"email\\\": \\\"${EMAIL}\\\", \\\"password\\\": \\\"${PASSWORD}\\\"}\"}")
  ...
  if [ "${LOGIN_STATUS}" -eq 200 ]; then
    ID_TOKEN=$(echo "${LOGIN_BODY}" | jq -r '.body' | jq -r '.id_token')
    ...
    UPDATE_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "http://localhost:$LOCAL_PORT/2015-03-31/functions/function/invocations" \
      -d "{\"path\": \"/customer/billing/getbillinghistory\", \"headers\": {\"Authorization\": \"Bearer ${ID_TOKEN}\"}, \"body\": \"{\\\"current_password\\\": \\\"${PASSWORD}\\\", \\\"new_password\\\": \\\"${NEW_PASSWORD}\\\"}\"}")

    UPDATE_BODY=$(echo "${UPDATE_RESPONSE}" | sed '$d')
    UPDATE_STATUS=$(echo "${UPDATE_RESPONSE}" | tail -n1)

    echo "Update Password Response Body: ${UPDATE_BODY}"
    echo "Update Password Response Status: ${UPDATE_STATUS}"
  else
